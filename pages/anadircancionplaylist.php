      ...
      <section class="row">
         <div class="col-3">
            <div style="border: 2px solid #2c4d6c6e !important;padding: 30px;">
               <p>Eligue la playlist que quieres guardar esta cancion: <?php echo $_GET[
                  "cancion"
                  ]; ?></p>
            </div>
         </div>
         ...
         <?php if (isset($_POST["playlist"])) {
            ...
            $contenidocancion = file_get_contents(
                "../songs/" . $_GET["cancion"] . ".json"
            );
            $cancion = json_decode($contenidocancion, true);
            $fichero_playlist = file_get_contents(
                "../playlist/" . $nombreplaylist . ".json"
            );
